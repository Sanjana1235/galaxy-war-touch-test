...
bulletsbuttonimg = loadImage("bullets_button.png") 
}
...
function setup() {
  ...
  mainspacecraft = createSprite(windowWidth/2,windowHeight-100);
  ...
  title.setCollider("rectangle",-350,-10,700,100)
    ...
    rightarrow = createSprite(windowWidth/2 + 250,windowHeight/2 + 300,20,20)
    rightarrow.addImage(rightarrowimg)
    rightarrow.visible = false
    rightarrow.scale = 0.3

    leftarrow = createSprite(windowWidth/2 - 259,windowHeight/2 + 300,20,20)
    leftarrow.addImage(leftarrowimg)
    leftarrow.visible = false
    leftarrow.scale = 0.3

    bulletsbutton = createSprite(windowWidth/2,windowHeight-30,20,20)
    bulletsbutton.addImage(bulletsbuttonimg)
    bulletsbutton.visible = false
    bulletsbutton.scale = 0.5
...
if (gamestate == "mainscreen"){
  ...
  if(mousePressedOver(play)||touches.length > 0){
    gamestate = "play"
    touches = [windowWidth/2,windowHeight/2]
...
if(gamestate == "play"){
  ...
  if (/*touches.length > 0 ||*/ mousePressedOver(rightarrow)){
    mainspacecraft.x = mainspacecraft.x + 20
    //touches = [windowWidth/2 + 250,windowHeight/2]
  }

  if (/*touches.length > 0 ||*/mousePressedOver(leftarrow)){
    mainspacecraft.x = mainspacecraft.x - 20
    //touches = [windowWidth/2 - 259,windowHeight/2]
  }
